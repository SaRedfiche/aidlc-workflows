...
export async function run(
  ...
  async function shellInvokesDynamicEvaluation(
    ...
    try {
      ...
      for (const segment of shellCommandSegments(command)) {
        const gitContext = gitCommandEnvironment(segment, cwd);
        for (const invocation of module.shellCommandInvocations(segment)) {
          const leaf = win32.basename(posix.basename(invocation.name));
          if (
            invocation.ambiguous ||
            isInterpreter(invocation.name) ||
            (/^busybox(?:\.exe)?$/i.test(leaf) && isInterpreter(invocation.args[0] ?? "")) ||
            (/^git(?:\.exe)?$/i.test(leaf) &&
            ...
            (hostExpression && directPowerShellHost.test(invocation.name)) ||
  ...
  async function touchesProtectedReviewerState(): Promise<boolean> {
    ...
    if (toolName === "Bash" && typeof command === "string") {
      try {
        ...
        if (concrete.some((path) => overlapsProtectedPath(path, protectedPaths, cwd))) return true;
      } catch {
