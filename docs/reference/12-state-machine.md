...
do not count. A recovered `GATE_REJECTED` row is reconciliation bookkeeping,
not a human decision, so it does not consume the human-presence turn. Bypass
...
| `HUMAN_TURN` | `hooks/aidlc-record-human-turn.ts` (+ per-harness prompt-submit adapters) | One per observed prompt-submit or answered-widget seam; the approval/interview gate requires one since the last human decision. Organic approval, rejection, answer, and summary-confirmation rows consume the turn; a revision-backstop `GATE_REJECTED` carrying `Recovered: true` does not. This is presence/freshness evidence, not an authenticated transcript or proof that later caller-supplied decision text was authored by the human. |
