...
| `resolved target for project pin <version> is missing or unavailable` | Recreate the validated machine-local target with `aidlc config --pin <version>`. The launcher fails closed instead of falling back to the active machine version. |
