...
describe("classifyTerminalCommand() - plugin utilities", () => {
  test("list, sync, select, validate, and build map to utility subcommands", () => {
    ...
    expect(
      ...
      args: ["claude", "out", "--plugin-root", "fixture"],
      display: "plugin build claude out --plugin-root fixture",
      source: "plugin-verb",
    });
  });
