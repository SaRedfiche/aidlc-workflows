...
//   - that EVERY one of the 87 VALID_EVENT_TYPES is accepted
...
// The 87 canonical event types, mirrored from aidlc-audit.ts VALID_EVENT_TYPES.
...
describe("VALID_EVENT_TYPES — every canonical type is accepted", () => {
  test("the mirrored list has 88 entries with no duplicates", () => {
    expect(VALID_EVENT_TYPES.length).toBe(88);
    expect(new Set(VALID_EVENT_TYPES).size).toBe(88);
  });
